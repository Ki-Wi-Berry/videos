...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
        let url="http://localhost:3007/api/user";
        let dataObject={phoneNumber:ruleForm.phone_number,password:ruleForm.password};
        axios.get(url,{params:dataObject}).then(function(res){
