...
<template>
  ...
  <video ref="Vvideo" src="https://test-streams.mux.dev/x36xhzz/x36xhzz.m3u8" controls="true" preload="auto"
    crossOrigin="anonymous" width="400" height="200" v-show="false">
    您的浏览器不支持 video 标签。
  </video>
...
const preloadVideoThumbs = () => {
  ...
  video.addEventListener('loadeddata', function () {
    ...
    const event = async function () {
      context?.drawImage(video, 0, 0, 200, 400);
      await canvas.toBlob(function (blob) {
        state.Thumbnails = URL.createObjectURL(blob);
      }, "image/jpeg");
      await state.Vvideo.removeEventListener('canplay', event);
      await document.body.removeChild(video);
    };
