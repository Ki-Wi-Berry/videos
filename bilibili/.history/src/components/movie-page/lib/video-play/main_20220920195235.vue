...
<template>
  ...
  <video ref="Vvideo" src="https://test-streams.mux.dev/x36xhzz/x36xhzz.m3u8" controls="true" preload="auto"
    crossOrigin="anonymous" width="400" height="200">
    您的浏览器不支持 video 标签。
  </video>
  <img :src="state.Thumbnails" alt="123">
</template>
<script lang="ts">
export default {
  name: "vue3VideoPlay",
...
const Vvideo: Ref<HTMLElement> = ref(null); // 视频播放器
...
// 进度条移动
const onProgressMove = async (ev, val) => {
  ...
  await new Promise(function (rsv) {
    const event = function () {
      context?.drawImage(state.Vvideo, 0, 0, 200, 400);
      canvas.toBlob(function (blob) {
        // let newImg = document.createElement("img");
        state.Thumbnails = URL.createObjectURL(blob);
        ...
        // state.Thumbnails = url;
      }, "image/jpeg")
      // state.Thumbnails = canvas.toDataURL('image/jpeg');
  ...
  // img.src=state.Vvideo;
  // props.Thumbnails =img.src;
  console.log(state.Thumbnails);
  // canvas.getContext("2d").drawImage(img, 0, 0, 200, 400);
