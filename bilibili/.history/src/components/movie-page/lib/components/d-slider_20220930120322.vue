  ...
  <div ref="refSlider" class="d-slider" @mousedown.stop="mouseDownHandle" @contextmenu="contextmenuHandle"
    ...
    <div class="d-slider__runway" @mousemove="mousemoveHandle">
      <div class="d-slider__cursor" :style="hoverStyle" v-show="props.hover">
        <div class="d-slider__tips" ref="refTips" :style="{ left: state.hoverImgLeft }" v-show="props.hoverText">
        {{props.hoverText }}</div>
        <img class="d-slider__img"  ref="refImg"   :style="{ left: state.hoverImgLeft }"
        :src="state.imgIndex<props.Thumbnails.length? props.Thumbnails[state.imgIndex]:state.loadingImg" alt="图片加载失败">
      </div>
...
const state = reactive({
  ...
  imgIndex:-1,       //预览图下标
  loadingImg:"./src/assets/image/24.gif"//加载中图片
})
// 获取当前位置的高度或宽度
...
// 鼠标移动事件
const mousemoveHandle = debounce(1000,false,(ev: MouseEvent) => {
