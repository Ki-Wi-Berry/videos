  ...
  <div ref="refSlider" class="d-slider" @mousedown.stop="mouseDownHandle" @contextmenu="contextmenuHandle"
    ...
    <div class="d-slider__runway" @mousemove="mousemoveHandle">
      <div class="d-slider__cursor" :style="hoverStyle" v-show="props.hover">
        <div class="d-slider__tips" ref="refTips"  v-show="props.hoverText">{{
        props.hoverText }}</div>
        <img class="d-slider__img" ref="refImg" :src="props.Thumbnails[2]" alt="123">
      </div>
...
const refTips: Ref<Nullable<HTMLElement | null>> = ref(null)
const refImg: Ref<Nullable<HTMLElement | null>> = ref(null)
...
const mousemoveHandle = (ev: MouseEvent) => {
  ...
  let refImgWidth = (refTips.value as HTMLButtonElement).clientWidth / 2;
  movePositon = ev.clientX - refSliderEl.getBoundingClientRect().left;
  // 如果当前往左的偏移量大于提示框宽度
  if (movePositon < refImgWidth) {
    state.hoverTipsLeft = (refImgWidth - movePositon) + 'px'
  } else if ((refSliderEl.clientWidth - movePositon) < refImgWidth) {
    // 如果当前往右的偏移量大于提示框宽度  （总宽度-当前移动位置）< tips一半的宽度
    state.hoverTipsLeft = (refSliderEl.clientWidth - movePositon) - refImgWidth + 'px'
  } else {
    state.hoverTipsLeft = '50%'
  }
