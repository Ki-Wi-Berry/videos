    ...
    >
      <ElFormItem prop="movieUrl" required class="form-select">
        ...
        >
          ...
          <div class="el-upload__text">点击或拖拽上传视频</div>
          <template #tip>
            <div class="el-upload__tip">
              请上传不超过 50M 后缀为 .mp4 的文件
            </div>
          </template>
        </ElUpload>
      </ElFormItem>
      ...
      <ElFormItem prop="imgUrl" required label-width="0px" class="form-select">
        ...
        >
          ...
          <template #tip>
            <div class="el-upload__tip">请上传不超过 2M 后缀为 .png 的文件</div>
          </template>
        </ElUpload>
      </ElFormItem>
...
import { InitUpload, UploadPart, FinishUpload,confirmUpload } from "../../api/request.js";
import type {
  UploadInstance,
  UploadProps,
  UploadRawFile,
    ...
    finishFilePartUploadFunc: async (md5) => {
      ...
      switch (uploadingFileType.value) {
        case UploadingFileType.movie:
          ruleForm.value.movieUrl = `${HOST}${data.path}`;
        case UploadingFileType.img:
          ruleForm.value.imgUrl = `${HOST}${data.path}`;
      }
